...
if (-not (Test-Path $payload)) {
    ...
    [Windows.Forms.MessageBox]::Show('The bundled manager payload was not found. Download a fresh manager executable.', 'Local VoiceMode Manager') | Out-Null
    exit 1
}
...
& powershell.exe -NoProfile -STA -ExecutionPolicy Bypass -File $manager
